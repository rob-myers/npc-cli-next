...
> In a nutshell: freely combining behaviours yields state-space explosions.
...
This already happens in very restricted settings e.g. [Conway's Game of Life](new-tab:https://en.wikipedia.org/wiki/Conway%27s_Game_of_Life).
...
<Card id="our-problem"> 
**Our Problem**: The underlying problem with a human Game Master.
...
Starting with the underlying problem and permitting a human to control the NPCs yields the second objective above.
But the second objective is only possible via real time control systems i.e. the first objective.
