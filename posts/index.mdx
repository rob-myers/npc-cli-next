...
Fully solving the underlying problem would require an "automated director" which does just that.
But let's not try to run before we can walk.
Instead assume the game involves two humans: the Player and the Game Master.
The Game Master controls the game in realtime,
ideally such that the Player believes each NPC is directly controlled by an individual human playing a role.
This is the problem we're concerned with.
