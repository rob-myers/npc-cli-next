...
For the underlying problem to be meaningful, human behaviour must be expressible.
...
In a nutshell:
freely combining behaviours inevitably leads to state-space explosions.<SideNote>Even in very restricted settings e.g. [Conway's Game of Life](new-tab:https://en.wikipedia.org/wiki/Conway%27s_Game_of_Life).</SideNote>
...
But there's a way to solve both these problems i.e. only react to the Player.
We could fabricate everything around them, as and when we need to.

This leads us to intermediate objectives, steps towards the underlying problem.
