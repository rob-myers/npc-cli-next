...
Human behaviour must be expressible for the underlying problem to be meaningful.
...
In a nutshell:
freely combining behaviours inevitably yields state-space explosions.<SideNote>Even in very restricted settings e.g. [Conway's Game of Life](new-tab:https://en.wikipedia.org/wiki/Conway%27s_Game_of_Life).</SideNote>
That is, the number of states becomes too large to effectively explore.
...
As an intermediate step we permit a human Game Master to control them.
