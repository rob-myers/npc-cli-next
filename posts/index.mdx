...
In particular, we don't have a solution but rather a strategy for solving it.
To understand this strategy it is best to see it in action, so let's begin.

### The Viewer

Depending on your screen size the Viewer will be either be at the bottom (small viewport) or on the right (larger viewport).

<Carousel
  items={[
    {
      img: IdleViewerSmallWiewportWebp,
      label: 'The Viewer in a small viewport',
      objectPosition: '0% 50%',
    },
    {
      img: IdleViewerSmallWiewportWebp,
