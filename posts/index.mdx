...
Our objectives arise naturally from the discussion above.
We want to avoid monolithic state-spaces by continually observing and reacting to the Player.
Then we're going to need real-time control systems,
and we're going to need a human (the Game Master) who observes/reacts using them.
We can now reinterpret the underlying problem.
We still want the NPCs to behave indistinguishably from human Players.
However, we permit NPCs to be controlled by a human Game Master rather than an automated director.
