...
import { error, info, debug, range, isInsideWebWorker, mapValues } from "../service/generic";
import { geomorph } from "../service/geomorph";
import { customThreeToTileCache, getTileCacheGeneratorConfig, getTileCacheMeshProcess, computeGmInstanceMesh, getTileTriangles } from "../service/recast-detour";
...
async function onRequestNav(msg) {
  ...
  const { toNavTris, toOffMeshEdges } = navForFloorDraw(
    gms,
    result.navMesh,
 ...
 */
function navForFloorDraw(gms, nav, offMeshLookup) {
  ...
  /** @type {NPC.FloorOffMeshEdges} */
  const toOffMeshEdges = mapValues(gmKeyToFirst, () => []);
