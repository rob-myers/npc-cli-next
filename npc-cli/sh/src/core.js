 ...
 * demoUi ui:base &
 * connectUi ui:base >/dev/null
 * connectUi ui:base inputs:'["npcKey"]'
 * ```
 * @param {NPC.RunArg} ct
 * @param {object} opts
 ...
 * }>}
 */
export async function connectUi(ct, opts = ct.api.jsArg(ct.args, { ui: 'uiKey', inputs: 'inputKeys' } )) {
...
export async function think({ api, args, w }, opts = api.jsArg(args, { npc: 'npcKey', of: 'thoughtKey' })) {
  ...
  if (bubble.popUp.opened === false) bubble.popUp.open();
